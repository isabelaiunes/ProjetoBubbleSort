...
# Loop2: Para cada elemento j no array de tamanho n-1 (compara os elementos de novo) #
...
def bubble_sort(arr):
    n = len(arr)
    # Para cada elemento i do array (Loop1) 
    for i in range(n):
        #Para cada elemento j do array (Loop2)
        for j in range(0, n-i-1):
            #Se elemento i for maior que elemento j (Condicional)
            if arr[j] > arr[j+1]:
